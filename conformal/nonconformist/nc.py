...
class QuantileRegErrFunc(RegressionErrFunc):
    ...
    def apply(self, prediction, y):
        n_intervals = prediction.shape[1] // 2
        y_lower = prediction[:, :n_intervals]
        y_upper = np.flip(prediction[:, n_intervals:], 1)  # We compute the errors from largest interval to smallest
        error_low = y_lower - y.reshape(-1, 1)
        error_high = y.reshape(-1, 1) - y_upper
...
class BaseModelNc(BaseScorer):
    ...
    def score(self, x, y=None):
		...
		"""
        prediction = self.model.predict(x)  # TODO : Correct if many quantiles
        n_test = x.shape[0]
        if self.normalizer is not None:
...
class RegressorNc(BaseModelNc):
    ...
    def __init__(self,
        ...
        self.gammas = gammas
        self.significance_t = significance_t

    def predict(self, x, nc, significance=None, **kwargs):
		...
		significance : float/list
			Significance level(s) (maximum allowed error rate) of predictions.
        ...
        no_conformal = kwargs['no_conformal'] if 'no_conformal' in kwargs else False
        n_test = x.shape[0]
        ...
        if significance is not None:
            ...
            if self.method == 'CQR':
                ...
                if isinstance(significance, float):
                    raise ValueError('No more useful')  ## TODO : Change that
                    intervals = np.zeros((x.shape[0], 2))
                    err_dist = self.err_func.apply_inverse(nc, significance)
                    err_dist = np.hstack([err_dist] * n_test)

                    if prediction.ndim > 1 and not no_conformal:  # CQR
                        intervals[:, 0] = prediction[:, 0] - err_dist[0, :]
                        intervals[:, 1] = prediction[:, -1] + err_dist[1, :]

                    elif prediction.ndim <= 1 and not no_conformal:  # regular conformal prediction
                        err_dist *= norm
                        intervals[:, 0] = prediction - err_dist[0, :]
                        intervals[:, 1] = prediction + err_dist[1, :]

                    elif no_conformal:
                        intervals[:, 0] = prediction
                        intervals[:, 1] = prediction

                else:  # list
                    intervals = np.zeros((x.shape[0], 2 * len(significance)))
                    # significance must be sorted
                    for j, alpha in enumerate(significance):
                        err_dist = self.err_func.apply_inverse(nc, alpha)

                        if prediction.ndim == 1 and not no_conformal:  # regular conformal prediction
                            intervals[:, j] = prediction - err_dist[0, j]
                            intervals[:, -(j + 1)] = prediction + err_dist[1, j]

                        if prediction.ndim > 1 and not no_conformal:  # CQR
                            intervals[:, j] = prediction[:, j] - err_dist[0, j]
                            intervals[:, -(j + 1)] = prediction[:, -(j + 1)] + err_dist[1, j]

                        elif no_conformal:
                            intervals[:, j] = prediction[:, j]
                            intervals[:, -(j + 1)] = prediction[:, -(j + 1)]

                return intervals

            elif self.method == 'ACI':
                significance_t = self.significance_t
                new_significance_t = significance_t.copy()
                assert len(significance_t) == len(
                    significance), 'Unequal number of adaptative significance levels and initial ones'
                intervals = np.zeros((x.shape[0], 2 * len(significance), len(self.gammas)))
                for j, alpha in enumerate(significance):
                    for idg, gamma in enumerate(self.gammas):
                        alpha_t = significance_t[j, idg]
                        err_dist = self.err_func.apply_inverse(nc, alpha_t)
                        ...
                        elif prediction.ndim == 1:  # Regular conformal prediction
                            y_lower = prediction - err_dist[0]
                            y_upper = prediction + err_dist[1]
                            err = 1 - (y_lower <= y_test).astype(int) * (y_test <= y_upper).astype(int)

                        elif prediction.ndim > 1:  # CQR
                            y_lower = prediction[:, j] - err_dist[0, j]
                            y_upper = prediction[:, -(j + 1)] + err_dist[1, j]
                            err = 1 - (y_lower <= y_test).astype(int) * (y_test <= y_upper).astype(int)
                        intervals[:, j, idg], intervals[:, -(j + 1), idg] = y_lower, y_upper

                        alpha_t = alpha_t + gamma * (alpha - err)
                        new_significance_t[j, idg] = alpha_t
                self.significance_t = new_significance_t
