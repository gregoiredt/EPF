...
class BaseIcp(BaseEstimator):
    ...
    def calibrate(self, x, y, increment=False):
        ...
        if self.conditional:
            raise ValueError('Not useful for the moment')  # TODO : DELETE THAT
            ...
            self.unsorted_cal_scores = {0: cal_scores}
...
class IcpRegressor(BaseIcp, RegressorMixin):
    ...
    def __init__(self, nc_function, condition=None):
        self.method = nc_function.method
        super(IcpRegressor, self).__init__(nc_function, condition)

    def predict(self, x, significance=None, **kwargs):
        """Predict the output values for a set of input patterns.
        ...
        for condition in self.categories:
            ...
            if np.sum(idx) > 0:
                ...
                if n_significance > 1:
                    if method =='CQR':
                        p = p.reshape((x.shape[0], 2, -1))
                        prediction = np.zeros((x.shape[0], 2, n_significance))
                        prediction[idx, :, :] = p
                    elif method == 'ACI':
                        # (x.shape[0], 2* n_significance, n_gammas) ->  (x.shape[0], 2, n_significance, n_gammas)
                        p = p.reshape((x.shape[0], 2, n_significance, -1))
